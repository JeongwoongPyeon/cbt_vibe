from app.schemas.question import GenerateQuestionsRequest, ImportImagesRequest, QuestionDraft
...
def validate_imported_question(
    ...
    if request.type != "auto" and question.type != request.type:
        errors.append("문제 유형이 요청과 일치하지 않습니다.")

    if not question.stem.strip():
        errors.append("문제 지문을 확인해야 합니다.")

    if question.type == "short_answer":
        if question.choices:
            errors.append("단답형 문제에는 보기가 없어야 합니다.")
    elif len(question.choices) != expected_count:
        errors.append(f"보기 {expected_count}개를 확인해야 합니다.")
    elif len({normalize_for_compare(choice) for choice in question.choices}) != expected_count:
        errors.append("보기에는 중복 항목이 없어야 합니다.")

    if not question.answer.strip():
        errors.append("정답을 확인하거나 직접 입력해야 합니다.")
        ...
        if not valid_numeric and not valid_text:
            errors.append("추출된 정답이 보기와 일치하지 않습니다.")
